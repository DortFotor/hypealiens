...
const getAccount = async()=>{
  ...
  , sendTransaction = async()=>{
    const e = await contract.methods.buyTokens(accounts[0], input.value).encodeABI()
      , t = (1e18 * price).toString(16)
      , n = (1e5 * +input.value).toString(16);
    ethereum.request({
        ...
        toastr.error("Transaction failed due to gas. Please try again.", "TXN ERROR")
    }
    )).catch((e=>console.error))
}
;
input.addEventListener("input", (()=>{
    +input.value < 1 ? button.disabled = !0 : +input.value >= 1 && (button.disabled = !1),
    price = .15 * +input.value,
